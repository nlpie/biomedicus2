...
public class AlcoholKindCandidateDetector implements KindCandidateDetector {
    ...
    public boolean isSocialHistoryHeader(List<ParseToken> headerTokens) {
        ...
        if ( strHeader.matches("(?is).*\\b" + strHeaderList + "\\b.*") ) {return true;}
    ...
    public boolean isSocialHistorySentence(List<ParseToken> sectionTitleTokens, List<ParseToken> sentenceTokens) {
        ...
        if ( !strSentence.matches("(?is).*\\b(drink|drank|glass|bottle|drinks)\\b.*")
                && !strSentence.matches("(?is).*\\b(alcohol-|blood +alcohol|good +spirit|Alcoholic +Dementia|eating +or +drinking|cirrhosis|benzyl +Alcohol|alcohol +intake|Alcohol/Drug|EtOH +withdrawal|alcoholic +hepatitis)\\b.*")
                )
        { return true; }
