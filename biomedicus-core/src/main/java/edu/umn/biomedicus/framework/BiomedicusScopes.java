...
/**
 * Contains the Guice scopes and contexts that store their scoped objects.<br/> <br/> There is one
 * default scope:<br/> <ul> <li> Processor Scope: contains objects that are one instance per
 * configured document processor. For example, any processor settings. </li>
 * </li> </ul> <br/> The primary method for running
 * code in scopes are {@link #createProcessorContext(Map)}.
 *
 * @author Ben Knoll
 * @since 1.6.0
 */
public final class BiomedicusScopes {
  ...
  /**
   * The processor scope. Used when binding objects to cause them to have a single instance per
   * processor type.
   */
  ...
  /**
   * Creates and returns a processor context that can be used to re-enter a processor scope multiple
   * times.
   *
  ...
  /**
   * A provider which represents an object that will be provided by seeding into the context via the
   * map arguments on {@link #createProcessorContext(Map)}. Modules can bind objects to this
   * provider in order to satisfy injector dependencies prior to the creation of the scopes. The
   * binding can only be injected inside a scope, and any provision request outside of a
   * {@link ContextScope} will cause an {@link IllegalStateException}.
   *
  ...
  /**
   * A context object, stores a ref to the context {@link ThreadLocal} and the map of objects to
   * provide within the scope.<br/> <br/> {@link #call(Callable)} causes the current thread to enter
   * the scope so that objects from the map are provided when scoped objects are needed.
   */
  public static final class Context {
    ...
    /**
     * Runs the callable inside of a context, providing objects from that context for injected types
     * within the context.
     *
  ...
  /**
   * The context scope. Provides scoped objects from the {@link Context} object for the current
   * thread.
   */
