...
/**
 * A template for a subsection of text, or a span. Indexes are in standard java string format, end
 * exclusive, i.e. the substring represented by this TextLocation will be the same as calling {@link
 * String#substring(int, int)} with the values of {@link #getBegin()} and {@link #getEnd()}.
 *
 * @since 1.5.0
 */
public interface TextLocation {
  ...
  /**
   * Converts an index with the same basis (0-index) as this span to use this span's begin as a
   * basis.
   *
  ...
  /**
   * Converts an index with this span's begin as a basis to an index with the same basis (0-index)
   * as this span.
   *
   * @param index the index to convert
  ...
  /**
   * Converts a span with this span's begin as a basis (0-index) to have the same basis as this
   * span.
   *
  ...
  /**
   * Returns true if this span contains the specified span, i.e. that the other span's begin is
   * after or the same as this span's begin, and the other span's end is before or the same as this
   * span's end.
   *
  ...
  /**
   * Determines whether all of the indexes in this span are in the specified sorted array of
   * indexes. Used for exclusion / ignore lists.
   *
   ...
   * @return new span from the end of this span up to the end of the other span
   */
