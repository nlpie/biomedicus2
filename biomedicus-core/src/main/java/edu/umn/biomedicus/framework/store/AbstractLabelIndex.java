...
public abstract class AbstractLabelIndex<T> implements LabelIndex<T> {
    ...
    public Optional<Label<T>> withTextLocation(TextLocation textLocation) {
        Iterator<Label<T>> it = insideSpan(textLocation)
                .filter(textLocation::spanEquals).iterator();
        if (it.hasNext()) {
    ...
    public Stream<Label<T>> stream() {
        ...
        Spliterator<Label<T>> spliterator = Spliterators
    ...
    public LabelIndex<T> inflate() {
        return new DefaultLabelIndex<>(this);
    }
}
