...
/**
 * A rtf document for modification, contains indices to all the symbols, which are entire control
 * words or characters in the rtf document.
 *
 * @author Ben Knoll
 * @since 1.3.0
 */
public class SymbolIndexedDocument {
  ...
  /**
   * A list of the symbols in the original document. A symbol is a character or control word in the
   * rtf document.
   */
  private final List<SymbolLocation> symbolLocations;

  /**
   * A map from destination names to maps from character indexes in that destination to symbol
   * indexes.
   */
  ...
  /**
   * Returns the index of the symbol from the character index and the destination that a character
   * occurs in.
   *
